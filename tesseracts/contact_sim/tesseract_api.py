...
class InputSchema(BaseModel):
    ...
    dt: Float64 = Field(description="Integrator step size.", default=1e-3)
    n_samples: int = Field(description="Trajectory sample rows returned for visualization.", default=0)


class OutputSchema(BaseModel):
    qf: Differentiable[Array[(4,), Float64]] = Field(description="State (x, y, vx, vy) at t_final.")
    impact_x: Differentiable[Array[(MAX_EVENTS,), Float64]] = Field(
        description="x-coordinate of each impact, NaN-padded to MAX_EVENTS."
    )
...
def _run(inputs: InputSchema, want_sens: bool):
    ...
    qf, Jqf, impact_x, Jimp, nev, traj = jl.ContactSolver.run_solver(
        theta,
        float(inputs.drag),
    ...
    J = np.vstack([np.asarray(Jqf), np.asarray(Jimp)]) if want_sens else None
    return (
        np.asarray(qf, dtype=np.float64),
        np.asarray(impact_x, dtype=np.float64),
        int(nev),
...
def apply(inputs: InputSchema) -> OutputSchema:
    qf, impact_x, nev, traj, _ = _run(inputs, want_sens=False)
    return OutputSchema(qf=qf, impact_x=impact_x, n_events=nev, traj=traj)
...
def abstract_eval(abstract_inputs):
    return {
        ...
        "traj": ShapeDType(shape=(int(abstract_inputs.n_samples), 5), dtype="float64"),
    }
