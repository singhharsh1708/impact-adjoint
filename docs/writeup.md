...
exists because of those impacts: a bounce separator that sorts particles by
...
wherever the dynamics are event-driven — contact, impact, switching,
thresholds. The parameter-dependence of the *event time* contributes a term
...
this solver. The honest repair inside pure JAX — interpolate the crossing
time from the guard and reset at the interpolated state — recovers a
converging gradient, and that is precisely the point: the repair *is*
first-order event-time sensitivity machinery, hand-implemented, with
...
Cost model, stated plainly: the sensitivities are *forward*-variational, so
...
saltation-VJP endpoints over HTTP — ~10,000 solver calls per chain. Two
...
numbers — the ensemble gradient is exact) the design objective. Two results:
...
(relative drift 5e-13 across five impacts — RK4 is exact on ballistic arcs
