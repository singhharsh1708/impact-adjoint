...
A 73-second walkthrough: [docs/demo.mp4](docs/demo.mp4). It is generated by
`python scripts/make_demo_video.py`, which reads every number it shows from
`experiments/results.json` and runs the boundary proof live, so the video
agrees with the artifacts for the same reason the figures do.
